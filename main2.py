...
input = dataset[:training_data_count, 0: girdi_sayisi]
target = dataset[:training_data_count, girdi_sayisi : girdi_sayisi + cikti_sayisi]


targetClassified = np.zeros(shape=(input.shape[0], 4))
print(input.shape)
for i in range(input.shape[0]):
    if target[i] == 1:
        targetClassified[i] = np.array([1, 0, 0, 0])
    elif target[i] == 2:
        targetClassified[i] = np.array([0, 1, 0, 0])
    elif target[i] == 3:
        targetClassified[i] = np.array([0, 0, 1, 0])
    elif target[i] == 4:
        targetClassified[i] = np.array([0, 0, 0, 1])
...
for i in range(predictions.shape[0]):
    if np.array_equal(predictions[i], np.array([1,0,0,0])):
        predictionResults[i] = 1
    elif np.array_equal(predictions[i], np.array([0,1,0,0])):
        predictionResults[i] = 2
    elif np.array_equal(predictions[i], np.array([0,0,1,0])):
        predictionResults[i] = 3
    elif np.array_equal(predictions[i], np.array([0,0,0,1])):
...
for x, y in zip(predictionResults, dataset[training_data_count:training_data_count+testing_data_count, girdi_sayisi:girdi_sayisi+cikti_sayisi]):
    x = int(np.round(x[0]))
    if int(x) == y:
